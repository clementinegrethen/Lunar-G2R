...
if __name__ == "__main__":
    argument_parser = ArgumentParser(prog='Absnav_simulator',
    ...
    argument_parser.add_argument("-l", "--lmdb", action='store_true', help="Gen 8bit image + depth map")
    argument_parser.add_argument("-t", "--traj", action='store_true', help="Gen only traj.csv")
    argument_parser.add_argument("-n", "--image_name", type=str, default="image", help="Float images prefix name when saving")
    ...
    for i in frame_range:
        ...
        if args.traj:
            ...
            # gen point cloud
            depth_map = renderer.client.getDepthMap()
            los_map = renderer.client.getLOSMap()
            pc = los_map.reshape(-1, 3) * depth_map.reshape(-1, 1)
            valid = np.isfinite(np.sum(pc, 1))
            point_cloud = pc[valid, :]
            import open3d as o3d
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(pc)
            o3d.io.write_point_cloud(f"{outdir_name}/pc_{i:07d}.ply", pcd)
            ...
            cv2.imwrite(f"{Path(outdir_name)}/image{i:07d}.tif", float_image)
